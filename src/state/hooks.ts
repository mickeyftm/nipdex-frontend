...
import { FarmCategory, Team } from 'config/constants/types'
...
export const useBusdPriceFromPid = (pid: number, type): BigNumber => {
  const farm = useFarmFromPid(pid, type)
...
export const useBusdPriceFromToken = (tokenSymbol: string): BigNumber => {
  ...
  const tokenPrice = useBusdPriceFromPid(tokenFarm?.pid, tokenFarm?.type)
  return tokenPrice
}

export const useLpTokenPrice = (symbol: string) => {
  const farm = useFarmFromLpSymbol(symbol)
  const farmTokenPriceInUsd = useBusdPriceFromPid(farm.pid, farm.type)
...
export const usePriceBnbBusd = (): BigNumber => {
  const bnbBusdFarm = useFarmFromPid(2, FarmCategory.BONE)
  return new BigNumber(bnbBusdFarm.quoteToken.busdPrice)
}

export const usePriceGboneBusd = (): BigNumber => {
  const cakeBnbFarm = useFarmFromPid(4, FarmCategory.GBONE)
  return new BigNumber(cakeBnbFarm.token.busdPrice)
}

export const usePriceBoneBusd = (): BigNumber => {
  const boneBnbFarm = useFarmFromPid(1, FarmCategory.BONE)
  return new BigNumber(boneBnbFarm.token.busdPrice)
}
...
// calc total locked value
export const useTotalLockedValue = () => {
  const farms = useFarms()
  const data = {
    total: BIG_ZERO,
    farmAndPool: BIG_ZERO,
    vault: BIG_ZERO 
  }
  ...
  for( let i = 0; i < farms.data.length; i++) {
    ...
    if (farm.lpTotalInQuoteToken) {
      const val = new BigNumber(farm.lpTotalInQuoteToken).times(farm.quoteToken.busdPrice)

      data.total = data.total.plus(val)
      data.farmAndPool = data.farmAndPool.plus(val)
    }
  }
  return data
}
