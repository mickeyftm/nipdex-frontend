  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      <TicketInput
        ...
        availableSymbol="GBONE"
      />
      <div>
        <Tips>{t('1 Ticket = %lotteryPrice% GBONE', { lotteryPrice: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {t('Ticket purchases are final. Your GBONE cannot be returned to you after buying tickets.')}
        </Announce>
        <Final>{t('You will spend: %num% GBONE', { num: cakeCosts(val) })}</Final>
      </div>
      <ModalActions>
